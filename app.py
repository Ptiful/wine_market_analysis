...
with chart1:
    ...
    ten_wines_to_increase = pd.read_sql_query(
        open("src/1 - 10_wines_to_increse_sales.sql", "r").read(),
        conn,
    )
...
with chart2:
    ...
    country_to_prioritize = pd.read_sql_query(
        open("src/2 - Country_to_prioritize.sql", "r").read(), conn
    )
...
price_for_the_best_winery = pd.read_sql_query(
    open("src/3 - Price_to_best_winery.sql", "r").read(),
    conn,
)
...
with chart3:
    ...
    grapes_and_wines = pd.read_sql_query(
        open("src/5 - Grapes_and_wines.sql", "r").read(), conn
    )
...
with chart4:
    ...
    country_leaderbord = pd.read_sql_query(
        open("src/6 - Country_leaderboard.sql", "r").read(), conn
    )
...
with chart5:
    ...
    country_leaderbord_vintages = pd.read_sql_query(
        open("src/6.1 - Country_leaderboard_vintage.sql", "r").read(), conn
    )
...
with chart6:
    ...
    top_5_Cabernet_Sauvignon_like = pd.read_sql_query(
        open("src/7 - Top_5_for_Sauvignon.sql", "r").read(), conn
    )
